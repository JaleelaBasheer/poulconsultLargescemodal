...
function FbxToGlbConverter() {
  ...
  const processModels = async () => {
      ...
      try {
        const fbxObject = await new Promise((resolve, reject) => {
          fbxLoader.load(
            ...
            (object) => {
              // Update bounding box for this object
              const objectBoundingBox = new THREE.Box3().setFromObject(object);
              cumulativeBoundingBox.union(objectBoundingBox); // Union with cumulative bounding box
              resolve(object);
            },
            (xhr) => {
        ...
        // Modify object materials for conversion
        fbxObject.traverse((child) => {
          if (child.isMesh) {
            child.material = new THREE.MeshStandardMaterial({ color: 0xcccccc });

            // Remove vertex colors and UVs
        ...
        const glbData = await new Promise((resolve, reject) => {
         ...
         // Load the converted glb for rendering with compression
        ...
        glbObject.traverse((child) => {
          if (child.isMesh) {
           ...
           // Generate a UUID and take the first 7 characters
          const uuid = uuidv4().substring(0, 7);
          const boundingBox = new THREE.Box3().setFromObject(child);
         // Create a unique ID for the mesh
         const meshId = `mesh-${uuid}`;
         console.log(child.position)  // Create a unique ID based on the current length
            child.userData.id = meshId; 
            child.userData.position=boundingBox.getCenter(new THREE.Vector3())// Set the ID in userData
            child.userData.size=boundingBox.getSize(new THREE.Vector3()).length() / 2

           // Push the mesh into the objects array
           objects.push(child);         
           }
        });
        const glbBlob = new Blob([glbData], { type: "application/json" });
    ...
    // After processing all files, compute the bounding box size and center
    ...
    objects.forEach((obj,index) => {
        ...
        // Insert the bounding box and the object with its id into the octree
        octreeRef.current.insert({
          position: boundingBox.getCenter(new THREE.Vector3()),  // Use center of the bounding box as position
          radius: boundingBox.getSize(new THREE.Vector3()).length() / 2,
          userData: {
            id: obj.userData.id,  // Store the id
            index:index,
            mesh: obj             // Store the actual mesh object
          }
        });
      });

    // Send bounding box data to parent
